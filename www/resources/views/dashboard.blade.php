...
@section('content')
    ...
    <h3 style="color: #5A378C;">PROJE DURUMLARI</h3>
    <hr style="border-color:black;">
    <p class="small" style="color: #190F26;"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Fikir aşamasında olan projeler grafik verilerine dahil değildir.</p>
    <p class="small" style="color: #190F26;"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Proje <b>"Oranı"</b>, oylamada projeye verilen puanları baz alarak hesaplanan "Proje Değerlendirme Oranıdır."</p>
    <p class="small" style="color: #190F26;"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> İptal edilen projeler gösterilmez.</p>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header" style="background-color: white;border: 5px solid #660066">
                        ...
                        <h5 class="card-title"><b>Proje Sahibi: </b>{{$users->where('id',$project->user_id)->value('name')}}  /
                            <b style="color:#ff0000;">
                                Oran: %{{ round((($votes->where('project_id',$project->id)->where('privilege_id','<>',3)->sum('vote')+($votes->where('project_id',$project->id)->where('privilege_id',3)->sum('vote')*10))*100) / (($voteQuestions->where('project_id',$project->id)->where('state',1)->count()*5)*(($votes->where('project_id',$project->id)->where('privilege_id','<>',3)->unique('user_id')->count())+($votes->where('project_id',$project->id)->where('privilege_id',3)->unique('user_id')->count()*10))),0) }}
                            </b>
                        </h5>
                    </div>
